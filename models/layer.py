'''
* @name: layer.py
* @description: Basic layer of the model. Note: Some code references Vision Transformer (VIT, https://github.com/gupta-abhay/pytorch-vit).
'''
...
class Attention(nn.Module):
    ...
    def forward(self, q, k, v, return_attn=False):
        ...
        if return_attn:
            return out, attn  # [B, heads, tgt_len, src_len]
        else:
            return out
...
class HhyperLearningEncoder(nn.Module):
    def __init__(self, dim, depth, heads, dim_head, dropout=0.):
        ...
        #self.g_t_values = []  # 存储整个 Encoder 过程中的 G_t
...
class CrossTransformerEncoder(nn.Module):
    ...
    def forward(self, source_x, target_x, return_attn=False):
        ...
        if return_attn:
            return target_x, all_attn  # [depth][B, H, tgt_len, src_len]
        else:
            return target_x
...
class CrossTransformer(nn.Module):
    ...
    def forward(self, source_x, target_x, return_attn=False):
        ...
        if return_attn:
            x_s2t, attn_weights = self.CrossTransformerEncoder(source_x, target_x, return_attn=True)
            return x_s2t, attn_weights
        else:
